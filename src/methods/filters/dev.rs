use crate::{
    EnsembleModel, EnsembleState, ModelError, math::quantiles, methods::filters::ParticleFilter,
    noise::NullNoise, obs::Obs,
};
use log::debug;
...
impl<T, OC, OD, M, const P: usize, const D: usize> ParticleFilter<T, OC, OD, M, P, D>
where
    M: EnsembleModel<T, P, D> + Sync,
    T: RealField + Sum,
    OC: Scalar + Sync,
    ...
    StandardNormal: Distribution<T>,
{
        ...
        OF: Fn(&M, &OC, &SVectorView<T, D>, &M::FMST, &M::CSST) -> Result<OD, ModelError<T>> + Sync,
    {
        ...
        let mut params = self.ensbl.params().clone_owned();
        ...
        params
            ...
            .for_each(|mut chunk| {
                chunk.iter_mut().for_each(|(idx, new_col)| {
                    ...
                    new_col.set_column(0, &self.model.prior_domain().clamp(&view));
                });
            });

        let mut temp_ensbl = EnsembleState::new(params, None, None);
        ...
        let mutated = temp_obs_ensbl
            ...
            .zip(self.ensbl.params_mut().par_column_iter_mut())
            .zip(temp_ensbl.params().par_column_iter())
        ...
        debug!(
            ...
            q_mid,
            q_hgh,
            T::from_f64((self.ensbl.len() * self.obs_ensbl.len()) as f64 / 1e6).unwrap(),
        ...
        self.iterations += 1;
        self.random_seed += 1;
        self.total_runs += self.ensbl.len();
